mutable struct{U}
    ...
    from this odd circuit are marked, i.e. node.is_marked == true. When a node on this circuit is
    traversed, we set node.is_processed to true. When a (+, +) inner edge is encountered, we can
